...
async function handleTextMessage(ctx) {
    ...
    // 1. Check Sheet ID
    let sheetId = (0, userSettingRepo_1.getUserSheet)(userId) || botConfig_1.config.defaultSpreadsheetId;
    ...
    try {
        // 2. Query Dictionary
        const wordInfo = await (0, dictionaryService_1.fetchWordInfo)(text);
        if (!wordInfo) {
        ...
        if (wordInfo.definition) {
            meaningVi = await (0, translateService_1.translateText)(wordInfo.definition);
        }
        ...
        const success = await (0, googleSheetService_1.appendToSheet)(sheetId, row);
        if (!success) {
